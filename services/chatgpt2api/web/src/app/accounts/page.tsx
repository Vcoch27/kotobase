...
function formatRestoreAt(value?: string | null) {
  ...
  const relative = diffMs > 0 ? `Còn ${days}d ${hours}h` : "Đã đến thời gian phục hồi";
...
function AccountsPageContent() {
  ...
  const loadModels = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Tải danh sách mô hình thất bại";
      toast.error(message);
    } finally {
  ...
  const handleRefreshAccounts = async (accessTokens: string[]) => {
    ...
    // Calculate base counts for non-selected accounts (for stat card linkage)
    ...
    // Show progress bar (current task only, no category stats)
    ...
    try {
        ...
        const pollTimer = setInterval(async () => {
          try {
            ...
            if (p.done) {
              ...
              // Update stat cards in real-time: base + accumulated refresh results
      ...
      if (relogined > 0) {
        setProgress({
          ...
          current: 0,
          total: relogined,
          message: `Đang thử xóa trạng thái bất thường cho ${relogined} tài khoản`,
          email: "",
        });
          ...
          const timer = setInterval(() => {
            ...
            if (reCount >= relogined) {
              ...
              setProgress({
                ...
                current: relogined,
                total: relogined,
                message: "Đã hoàn tất xóa trạng thái bất thường",
                email: "",
              });
      ...
      if ((data.errors ?? []).length > 0) {
        ...
        toast.error(
          `Làm mới thành công ${data.refreshed} tài khoản, thất bại ${(data.errors ?? []).length} tài khoản${firstError ? `, lỗi đầu tiên: ${firstError}` : ""}`,
        );
      } else {
        toast.success(`Làm mới thành công ${data.refreshed} tài khoản${relogined > 0 ? `, đã kích hoạt ${relogined} tài khoản đăng nhập lại` : ""}`);
      }
    } catch (error) {
  ...
  const handleReLogin = async (accessTokens: string[]) => {
    if (accessTokens.length === 0) {
      toast.error("Vui lòng chọn tài khoản để khôi phục");
      return;
    }

    // Only handle abnormal accounts, filter out non-abnormal
    ...
    if (abnormalTokens.length === 0) {
      toast.error("Không có tài khoản bất thường nào trong lựa chọn");
      return;
    }
    ...
    // Show progress bar (real progress)
    ...
    try {
        ...
        const pollTimer = setInterval(async () => {
          try {
            ...
            if (p.done) {
              ...
              // Update stat cards in real-time: base + processed recovery results
              ...
              for (const r of results) {
                ...
                // "异常" or "跳过": keep abnormal status unchanged
              }
              setRefreshSummary({
                total: accounts.length,
                active: runningActive,
      ...
      setProgress({
        ...
        current: total,
        total,
        message: "Đã khôi phục xong",
        email: "",
      });
      setTimeout(() => setProgress({ visible: false, current: 0, total: 0, message: "", email: "" }), 800);

      toast.success(`Quy trình khôi phục đã hoàn tất`);
    } catch (error) {
      setProgress({ visible: false, current: 0, total: 0, message: "", email: "" });
      setRefreshSummary(null);
      ...
      <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            Làm mới tất cả thông tin & hạn mức
          </Button>
          <AccountImportDialog
            disabled={isLoading || isRefreshing || isDeleting}
            onImported={(items) => {
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Thủ công thay đổi trạng thái và proxy riêng của tài khoản.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                <Input
                  ...
                  placeholder="Để trống để dùng proxy chung, ví dụ http://127.0.0.1:7890"
          ...
          <CardContent className="p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {availableModels.length > 0 ? (
                availableModels.map((model) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      toast.success("Đã sao chép tên mô hình");
                    }}
                ...
                <span className="text-sm text-stone-400">Hiện không có mô hình nào</span>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
            <CardContent className="flex flex-col items-center justify-center gap-3 px-6 py-14 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-stone-500">Đồng bộ danh sách và trạng thái tài khoản từ backend.</p>
              </div>
            </CardContent>
          </Card>
        ) : null}
            ...
            <div className="flex flex-col gap-3 border-b border-stone-100 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap items-center gap-2 text-sm text-stone-500">
                ...
                <Button
                  ...
                  title="Thử đăng nhập bằng mật khẩu để khôi phục tài khoản"
                >
                  {isRelogining ? <LoaderCircle className="size-4 animate-spin" /> : <LogIn className="size-4" />}
                  Khôi phục tài khoản bất thường
                </Button>
                <Button
                  variant="ghost"
              ...
              <table className="w-full min-w-[1000px] text-left">
                <thead className="border-b border-stone-100 text-[11px] text-stone-400 uppercase tracking-[0.18em]">
                  <tr>
                    ...
                    <th className="w-18 px-4 py-3">Đang tạo</th>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          ...
                          >
                            ...
                            {account.status === "正常" ? "Bình thường" : account.status === "限流" ? "Giới hạn" : account.status === "异常" ? "Bất thường" : "Vô hiệu"}
                          </Badge>
                        </td>
                        <td className="px-4 py-3">
                          ...
                          {(() => {
                            ...
                            return (
                              <span
                                ...
                                title={
                                  inflight > 0
                                    ? "Số lượng ảnh đang được tạo. Nếu giá trị này > 0 liên tục khi pool rảnh, có thể slot đã bị rò rỉ và tài khoản bị loại khỏi lịch trình"
                                    : "Hiện không có tác vụ tạo ảnh đang chạy"
                                }
                              >
                                {inflight}
                              </span>
              ...
              {!isLoading && currentRows.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-3 px-6 py-14 text-center">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-stone-500">Điều chỉnh bộ lọc hoặc từ khóa tìm kiếm và thử lại.</p>
                  </div>
                </div>
              ) : null}
            </div>

...
// Blob URL cache: avoid long base64 strings in DOM
...
export function ImageResults({
  ...
  // Only start timer when loading images exist
  ...
  const updateImageDimensions = (id: string, width: number, height: number) => {
    ...
    // Use ref to store, avoid React re-rendering cascade
    ...
    return (
      <div className="flex h-full min-h-[260px] items-center justify-center text-center sm:min-h-[420px]">
        <div className="w-full max-w-4xl">
          ...
          >
            Giữ lại lịch sử địa phương và trạng thái tác vụ trong cùng một cửa sổ, và bắt đầu chỉnh sửa không trạng thái mới từ kết quả có sẵn.
          </p>
        </div>
      </div>
    );
              ...
              <div className="flex justify-end">
                <div className="max-w-[90%] px-1 py-1 text-[14px] leading-6 text-stone-900 sm:max-w-[82%] sm:text-[15px] sm:leading-7">
                  <div className="mb-1.5 flex flex-wrap justify-end gap-2 text-[11px] text-stone-400 sm:mb-2">
                    <span>Lần {turnIndex + 1}</span>
                    <span>
                      {turn.mode === "edit" ? "Chỉnh sửa" : "Văn bản thành ảnh"}
                    </span>
                ...
                <div className="w-full p-1">
                  {turn.referenceImages.length > 0 ? (
                    <div className="mb-4 flex flex-col items-end">
                      <div className="mb-3 text-xs font-medium text-stone-500">Ảnh tham khảo lần này</div>
                        ...
                        {turn.referenceImages.map((image, index) => (
                          <div key={`${turn.id}-${image.name}-${index}`} className="flex flex-col items-end gap-2">
                            <button
                              ...
                              aria-label={`Xem trước ảnh tham khảo ${image.name || index + 1}`}
                            >
                              <img
                                src={image.dataUrl}
                                alt={image.name || `Tham khảo ${index + 1}`}
                  ...
                  <div className="mb-3 flex flex-wrap items-center gap-1.5 text-[11px] text-stone-500 sm:mb-4 sm:gap-2 sm:text-xs">
                    ...
                    {turn.status === "queued" ? (
                      <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-700">Đang chờ tác vụ trước trong cuộc trò chuyện hoàn tất</span>
                    ) : null}
                  </div>
                        ...
                        return (
                          <div key={image.id} className="break-inside-avoid">
                            ...
                            <div className="flex h-full min-h-16 flex-col items-center justify-center gap-1.5 px-2 py-2 text-center text-[11px] leading-4 text-rose-600 sm:gap-3 sm:px-6 sm:py-8 sm:text-sm sm:leading-6">
                              ...
                              <span className="line-clamp-2 sm:line-clamp-none">{image.error || "Tạo thất bại"}</span>
...
const PROGRESS_LABELS: Record<string, string> = {
  getting_account: "Xác nhận tài khoản",
  uploading: "Đang tải ảnh lên",
  bootstrapping: "Đang khởi tạo",
  getting_token: "Đang lấy token",
  preparing_conversation: "Đang chuẩn bị",
  starting_generation: "Đang bắt đầu tạo",
  generating: "Đang tạo",

      ...
      >
        ...
        <div className="relative p-6 bg-gradient-to-br from-indigo-50 dark:from-indigo-900/60 via-white dark:via-slate-900 to-purple-50 dark:to-purple-900/40 border-b border-slate-200 dark:border-slate-700/60 transition-colors flex-shrink-0">
          ...
          <div className="flex items-center gap-5">
            <div className="flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-tr from-amber-500 to-rose-500 text-white font-black text-4xl shadow-lg shadow-amber-500/20 border border-amber-300/30 flex-shrink-0">
              {character}
            </div>
        ...
        {loading ? (
          ...
          <form onSubmit={handleSave} className="p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1">
            ...
            {/* Mnemonic / Mẹo nhớ - Thu gọn textbox, chỉ hiện thẻ Highlight trực quan khi xem */}
            <div>
              <div className="flex items-center justify-between mb-1.5 flex-wrap gap-1">
                ...
                <button
                  type="button"
                  onClick={() => setIsEditingMnemonic(prev => !prev)}
                  className="inline-flex items-center gap-1 text-[11px] font-semibold text-amber-600 dark:text-amber-400 hover:underline"
                >
                  {isEditingMnemonic ? (
                    <>
                      <Eye className="w-3.5 h-3.5" /> Thu gọn
                    </>
                  ) : (
                    <>
                      <Pencil className="w-3.5 h-3.5" /> {mnemonic.trim() ? "Chỉnh sửa" : "Viết mẹo nhớ"}
                    </>
                  )}
                </button>
              </div>

              {/* Chế độ Xem (Mặc định khi đã có Mnemonic): Chỉ hiển thị nội dung Highlight, không lặp lại textbox */}
              {!isEditingMnemonic ? (
                mnemonic.trim() ? (
                  <div
                  ...
                  <button
                    type="button"
                    onClick={() => setIsEditingMnemonic(true)}
                    className="w-full py-3 px-3.5 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700/80 hover:border-amber-400 dark:hover:border-amber-500/60 text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50/40 dark:hover:bg-amber-500/5 transition-all text-left flex items-center justify-between"
                  >
                    <span>+ Thêm câu chuyện mẹo nhớ (Mnemonic)...</span>
                    <Pencil className="w-3.5 h-3.5 text-slate-400" />
                  </button>
                )
              ) : (
                /* Chế độ Chỉnh sửa: Hiện Textarea cùng quy tắc highlight */
                <div className="space-y-2">
                  ...
                  />
                  {mnemonic.trim() && (
                    <div className="p-3 rounded-xl bg-slate-100/80 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 transition-all">

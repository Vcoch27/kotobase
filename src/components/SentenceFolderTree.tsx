...
import { deleteSentenceFolder, updateSentenceFolder } from '@/app/actions/sentence';
import {
  Folder,
  ChevronRight,
  ChevronDown,
...
interface SentenceFolderItem {
  id: string;
  name: string;
  parentId: string | null;
  _count?: { folderSentences: number };
}

interface SentenceFolderTreeProps {
  folders: SentenceFolderItem[];
  selectedFolderId: string;
  onSelectFolder: (id: string) => void;
  onRefresh: () => void;
}

export function SentenceFolderTree({
  folders,
  selectedFolderId,
  onSelectFolder,
  onRefresh,
}: SentenceFolderTreeProps) {
  const [expandedFolders, setExpandedFolders] = useState<Record<string, boolean>>(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem('kotobase_sentence_expanded_folders');
  ...
  const [localFolders, setLocalFolders] = useState<SentenceFolderItem[]>([]);
  ...
  useEffect(() => {
    if (selectedFolderId && selectedFolderId !== 'all' && folders.length > 0) {
      const folderMap = new Map<string, SentenceFolderItem>();
      ...
      if (Object.keys(newExpanded).length > 0) {
        setExpandedFolders((prev) => {
          ...
          try {
            localStorage.setItem('kotobase_sentence_expanded_folders', JSON.stringify(updated));
          } catch (e) {}
          return updated;
        });
      }
  ...
  const handlePromptSubmit = async (e: React.FormEvent) => {
    ...
    if (promptModal.type === 'rename') {
      ...
      const res = await updateSentenceFolder(promptModal.targetId, newName);
      if (res.success) {
      ...
      setPromptModal((prev) => ({ ...prev, isOpen: false }));
      const res = await deleteSentenceFolder(promptModal.targetId);
      if (res.success) {
        toast.success('Đã xóa thư mục và mẫu câu bên trong!');
  ...
  const renderNode = (node: any, depth = 0) => {
    ...
    const sentenceCount = node._count?.folderSentences || 0;
      ...
      <div key={node.id}>
        <div
          ...
          className={`group flex items-center justify-between py-2 pr-2.5 rounded-xl transition-all cursor-pointer select-none border-b border-transparent mb-1
            ${isSelected 
              ? 'bg-indigo-50 dark:bg-indigo-500/10 border-indigo-100 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-300 shadow-sm' 
        ...
        >
          <div className="flex items-center gap-1.5 flex-1 min-w-0">
            ...
            <Folder className={`w-4 h-4 shrink-0 transition-colors ${isSelected ? 'text-indigo-500 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`} />
    ...
    <div className="flex flex-col h-full space-y-4">
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`p-1.5 rounded-lg ${selectedFolderId === 'all' ? 'bg-amber-200 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400' : 'bg-amber-100 dark:bg-amber-500/10 text-amber-600 dark:text-amber-500'}`}>
            <Folder className="w-4 h-4" />
          </div>
          <span className="text-sm font-bold">Tất cả mẫu câu</span>
        </div>
            ...
            <form onSubmit={handlePromptSubmit} className="p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm font-semibold text-slate-800 dark:text-slate-100 outline-none transition-all"
                />
              </div>
              ...
              {promptModal.type === 'delete' && (
                <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/30 border border-rose-100 dark:border-rose-900/50 flex gap-2.5 items-start">
                  ...
                  <p className="text-[11px] text-rose-600 dark:text-rose-400 font-medium leading-relaxed">
                    Hành động này sẽ xóa thư mục và <b className="font-bold">TOÀN BỘ mẫu câu</b> bên trong. Không thể khôi phục!
                  </p>
                </div>
              )}
